...
public class GemForgeMenu extends RecipeBookMenu<GemForgingRecipeInput, GemForgingRecipe> {
    ...
    private final IGemForgeContainer inventory;
    protected Level world;
    public ContainerData forgeData;

    public GemForgeMenu(int id, Inventory playerInventory) {
        this(
                id,
                playerInventory,
                new SimpleGemForgeContainer(), // dummy inventory
                new SimpleContainerData(SLOT_COUNT)
        );
    }

    public GemForgeMenu(int id, Inventory playerInventory, IGemForgeContainer inventory) {
        this(id, playerInventory, inventory, new SimpleContainerData(SLOT_COUNT));
    }

    public GemForgeMenu(int id, Inventory playerInventory, IGemForgeContainer inventory, ContainerData forgeData) {
        super(MMMenuTypes.GEM_FORGE_MENU.get(), id);
        MiningMaster.LOGGER.info("creating GemForgeMenu");
...
//    }
    ...
    public void fillCraftSlotsStackedContents(StackedContents itemHelperIn) {
        if (this.inventory instanceof StackedContentsCompatible) {
            ((StackedContentsCompatible)this.inventory).fillStackedContents(itemHelperIn);
        }
    }
...
//        }
        return recipe.value().matches(this.inventory.asRecipeInput(), this.world);
    }
...
//    @OnlyIn(Dist.CLIENT)
    ...
    class GemSlot extends Slot {
        public GemSlot(Container inventory, int index, int xIn, int yIn) {
            super(inventory, index, xIn, yIn);
        }
    ...
    class CatalystSlot extends Slot {
        public CatalystSlot(Container inventory, int index, int xIn, int yIn) {
            super(inventory, index, xIn, yIn);
        }
